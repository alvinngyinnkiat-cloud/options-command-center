...
function mapRiskSettings(row: RiskSettings | null): RiskSettingsSnapshot {
  if (!row) return MOCK_RISK_SETTINGS;
  return {
...
async function getRiskSettings(userId?: string): Promise<RiskSettingsSnapshot> {
  if (!isSupabaseConfigured() || !userId) {
    return MOCK_RISK_SETTINGS;
  }
...
export async function getRiskDashboardData(): Promise<RiskDashboardData> {
  ...
  let settings = MOCK_RISK_SETTINGS;
  ...
  if (isSupabaseConfigured()) {
    try {
      ...
      if (user) {
        settings = await getRiskSettings(user.id);
      }
    } catch {
      settings = MOCK_RISK_SETTINGS;
    }
  }
